...
import { ONBOARDING_ALLOWED_PATHS, ROUTE_MAP } from './RouteMap';
...
export function OnboardingGate({ children }: OnboardingGateProps) {
  ...
  if (ONBOARDING_ALLOWED_PATHS.has(location.pathname)) {
    if (location.pathname === ROUTE_MAP.studyOnboarding && data?.isOnboarded === true) {
      return <Navigate to={ROUTE_MAP.dashboard} replace />;
    }
    return <>{children}</>;
  }
