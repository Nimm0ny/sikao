...
export default function StudyToday() {
  ...
  function handleSkip(taskId: number) {
    patchTask.mutate({ id: taskId, status: 'skipped' }, {
      ...
      onError: (err) => { logger.error('skip task failed', { err: String(err) }); toast.error(STUDY_COPY.TODAY.SKIP_FAILED); },
    });
  }
        ...
        <div className='max-w-lg mx-auto px-4 py-8'>
          ...
          <div className='p-6 rounded-card' style={ts}>
            ...
            <div className='flex gap-2'>
              <button className='py-2 px-4 rounded-tiny font-medium' style={{ background: 'var(--accent-1)', color: 'var(--paper-1)', fontSize: 'var(--t-small)' }} onClick={() => void refetch()}>重试</button>
