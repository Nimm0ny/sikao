...
export function SectionA({
  ...
  readonly dailyHistory: ReadonlyArray<{ id: number; date: string; questionCount: number; completedAccuracy?: number | null }>;
  ...
  if (error) return <Banner variant="err" title="统计面板加载失败" description="请刷新后重试。" />;
  if (!history || history.summary.totalAttempts === 0) {
    return <EmptyState title="还没有练习记录" description="从每日一练、专项练习或套卷入口开始，Section A 会自动汇总你的历史与趋势。" />;
  }
        ...
        <div className={styles.historyList}>
          ...
          {history.recentSessions.map((session) => (
            <article key={String(session.sessionId)} className={styles.historyItem}>
              ...
              <span>{formatAccuracy(session.accuracyRate)} · {session.answeredQuestions}/{session.totalQuestions} 题</span>
            </article>
          ))}
        </div>
        <div className={styles.historyList}>
            ...
            {trend.slice(-4).map((point) => (
              <div key={point.date} className={styles.trendBarItem}>
                ...
                <div className={styles.trendBarTrack}>
                  <div className={styles.trendBarFill} style={{ width: `${Math.round(point.accuracy * 100)}%` }} />
                </div>
          ...
          {dailyHistory.map((item) => (
            <article key={item.id} className={styles.historyItem}>
              ...
              <span>
                {item.questionCount} 题 · {typeof item.completedAccuracy === 'number' ? formatAccuracy(item.completedAccuracy) : '未完成'}
              </span>
            </article>
          ))}
        </div>
...
export function CatalogSection({
  ...
  if (error) return <Banner variant="err" title="专项分类加载失败" action={{ label: '重试', onClick: onRetry }} />;
  if (items.length === 0) return <EmptyState title="暂无专项分类" description="当前范围没有可用的分类入口。" />;
          ...
          <div className={styles.catalogItems}>
            {children.map((item) => (
              <article key={item.id} className={styles.catalogItem}>
                ...
                <Button variant="secondary" onClick={() => onOpen(item)}>按此配置</Button>
              </article>
            ))}
          </div>
        </details>
...
export function PapersSection({
  ...
  readonly filters: { year?: number | null; region?: string | null; examType?: string | null; difficultyMin?: number | null; difficultyMax?: number | null };
  readonly sort: 'default' | 'recent' | 'year_desc';
  readonly onRetry: () => void;
  readonly onPatchFilters: (patch: { year?: number | null; region?: string | null; examType?: string | null; difficultyMin?: number | null; difficultyMax?: number | null }, segment?: 'xingce' | 'essay') => void;
  readonly onResetFilters: (segment?: 'xingce' | 'essay') => void;
  readonly onSort: (sort: 'default' | 'recent' | 'year_desc', segment?: 'xingce' | 'essay') => void;
  readonly onStart: (item: CatalogItemV2) => void;
}) {
  if (loading) return <Skeleton variant="text" lines={8} />;
  if (error) return <Banner variant="err" title="套卷列表加载失败" action={{ label: '重试', onClick: onRetry }} />;

  const yearOptions = Array.from(new Set(items.map((item) => item.year).filter((value): value is number => value != null)))
    .sort((a, b) => b - a)
    .map((value) => ({ value: String(value), label: `${value}` }));
  const regionOptions = Array.from(new Set(items.map((item) => item.region).filter((value): value is string => Boolean(value))))
    .map((value) => ({ value, label: value }));
  const examTypeOptions = Array.from(new Set(items.map((item) => item.examType).filter((value): value is string => Boolean(value))))
    .map((value) => ({ value, label: value }));

  return (
    <div className={styles.sectionBody}>
      <div className={styles.filterBar}>
        <Select value={filters.year != null ? String(filters.year) : 'all'} onChange={(value) => onPatchFilters({ year: value === 'all' ? null : Number(value) })} options={[{ value: 'all', label: '全部年份' }, ...yearOptions]} />
        <Select value={filters.region ?? 'all'} onChange={(value) => onPatchFilters({ region: value === 'all' ? null : value })} options={[{ value: 'all', label: '全部地区' }, ...regionOptions]} />
        <Select value={filters.examType ?? 'all'} onChange={(value) => onPatchFilters({ examType: value === 'all' ? null : value })} options={[{ value: 'all', label: '全部考试类型' }, ...examTypeOptions]} />
        {showDifficultyFilter ? (
          <Select
            value={
        ...
        />
        <Button variant="ghost" onClick={() => onResetFilters()}>重置筛选</Button>
      </div>

      {items.length === 0 ? (
        <EmptyState title="当前筛选没有套卷" description="调整年份、地区或排序后再试。" />
      ) : (
          ...
          {items.map((item) => (
            <article key={item.id} className={styles.paperCard}>
              ...
              <div className={styles.paperMeta}>
                {item.isCompleted ? <Badge variant="ok" size="sm">已完成</Badge> : <Badge variant="neutral" size="sm">未完成</Badge>}
                <Button variant="secondary" onClick={() => onStart(item)}>开始</Button>
              </div>
            </article>
          ))}
        </div>
