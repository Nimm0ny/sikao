...
def _assert_records_contract_shape(payload: dict) -> None:
    expected_section_status = "empty" if payload["total"] == 0 else "partial"
    assert set(payload.keys()) == {"summary", "sections", "actions", "items", "total", "page", "pageSize"}
    assert payload["summary"]["totalAttempts"] == payload["total"]
    assert payload["total"] >= len(payload["items"])
    assert payload["page"] == 1
    assert payload["pageSize"] == 20
    assert payload["sections"][0]["status"] == expected_section_status
    if payload["total"] == 0:
...
def test_dashboard_records_empty_contract_is_consistent(tmp_path: Path) -> None:
    with build_client(tmp_path) as (client, _app):
        ...
        response = client.get("/api/v2/dashboard/records")

        assert response.status_code == 200, response.text
        payload = response.json()
        _assert_records_contract_shape(payload)
        assert payload["summary"]["totalAttempts"] == 0
        assert payload["summary"]["xingceAttempts"] == 0
        assert payload["summary"]["essayAttempts"] == 0
        assert payload["summary"]["completedAttempts"] == 0
        assert payload["items"] == []
...
def test_dashboard_records_xingce_only_uses_single_aggregate_semantics(tmp_path: Path) -> None:
    with build_client(tmp_path) as (client, app):
        ...
        payload = response.json()
        _assert_records_contract_shape(payload)
        assert payload["summary"]["totalAttempts"] == 2
        assert payload["summary"]["xingceAttempts"] == 2
        assert payload["summary"]["essayAttempts"] == 0
        assert payload["summary"]["completedAttempts"] == 1
...
def test_dashboard_records_essay_only_uses_single_aggregate_semantics(tmp_path: Path) -> None:
    with build_client(tmp_path) as (client, app):
        ...
        payload = response.json()
        _assert_records_contract_shape(payload)
        assert payload["summary"]["totalAttempts"] == 1
        assert payload["summary"]["xingceAttempts"] == 0
        assert payload["summary"]["essayAttempts"] == 1
        assert payload["summary"]["completedAttempts"] == 1
        assert payload["summary"]["avgEssayScore"] == "72.50"
...
def test_dashboard_records_mixed_data_keeps_summary_items_and_total_in_sync(tmp_path: Path) -> None:
    with build_client(tmp_path) as (client, app):
        ...
        payload = response.json()
        _assert_records_contract_shape(payload)
        assert payload["summary"]["totalAttempts"] == 3
        assert payload["summary"]["xingceAttempts"] == 2
        assert payload["summary"]["essayAttempts"] == 1
        assert payload["summary"]["completedAttempts"] == 2
        assert payload["summary"]["avgEssayScore"] == "68.00"
...
def test_dashboard_records_support_paginated_items_with_global_total_and_summary(tmp_path: Path) -> None:
    with build_client(tmp_path) as (client, app):
        ...
        payload = response.json()
        _assert_records_contract_shape(payload)
        assert payload["summary"]["totalAttempts"] == 23
        assert payload["summary"]["xingceAttempts"] == 18
        assert payload["summary"]["essayAttempts"] == 5
        assert payload["summary"]["completedAttempts"] == 14
        assert payload["summary"]["avgEssayScore"] == "70.00"
...
def test_dashboard_records_unscored_essay_without_report_normalizes_to_pending(tmp_path: Path) -> None:
    with build_client(tmp_path) as (client, app):
        ...
        assert payload["summary"]["essayAttempts"] == 1
        assert payload["summary"]["completedAttempts"] == 0
        assert payload["summary"]["avgEssayScore"] is None
        assert payload["total"] == 1
        assert payload["items"][0]["kind"] == "essay_submission"
        assert payload["items"][0]["title"] == "Essay submission"
