...
def test_postgres_home_llm_service_reference_requires_two_global_slots(tmp_path: Path) -> None:
    with build_postgres_engine("sikao_llm_practice_ref_slots") as engine:
        ...
        with session_factory() as session:
            user = seed_user(session, name="Reference Slot User")
            session.add(
                LlmCallV2(
                    user_id=user.id,
                    purpose="question_generation",
                    prompt_version="question_generation@test",
                    provider="mock",
                    model="mock-model",
                    input_tokens=10,
                    output_tokens=20,
