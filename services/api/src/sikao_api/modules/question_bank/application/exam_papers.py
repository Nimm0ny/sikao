...
class ExamPaperService:
    ...
    def start_study_plan_session(
        ...
        *,
        ...
        """学习计划 task 跳转入口. 跟 retry-batch 区别:

        - 跳过错题本校验 (LLM 可能选不在错题本的新题)
        - cross-paper 路径含材料题仍 fail-fast (复用 _start_retry_cross_paper 限制)
        - paperCode 可选 (review_wrong 跨卷场景没 paperCode); cross-paper 分支
          下若 caller 传了 paperCode 仅 warn 不阻断 (FE wiring 不传, defensive)
        - mode=MODE_STUDY_PLAN (single-revision) / MODE_STUDY_PLAN_CROSS_PAPER
          (cross-paper). 让统计能区分 study_plan vs retry_wrong 两类入口.
        - 若 caller 传 paperCode + questionIds 同时, 单 revision 场景下做
          sanity assert 防 caller 拼错 (paper_code_mismatch 422)
        - questionIds 部分不存在 → ValidationError(422, code="question_not_found"),
          因为 list 元素无效是 client request 校验失败, 不是路由 404.

        副作用 (合理设计): 用户答错 LLM 推荐的"非错题本"题, 该题会进入 user
        错题本 (跟普通整卷做错同语义, mastery 自动 ensure-create).
        """
        unique_ids = self._validate_retry_batch_input(question_ids)
        questions = list(
            self.session.scalars(
        ...
        if not questions or len(questions) != len(unique_ids):
            raise ValidationError(
                "questionIds 含不存在或已删除的题", code="question_not_found"
            )

        revision_ids = {q.paper_revision_id for q in questions}
        if len(revision_ids) == 1:
            if paper_code is not None:
                actual_paper_code = next(iter(questions)).paper_revision.paper.paper_code
                if actual_paper_code != paper_code:
                    raise ValidationError(
                        f"paperCode {paper_code!r} 跟 questionIds 推出的 paper "
                        f"{actual_paper_code!r} 不一致",
                        code="paper_code_mismatch",
                    )
            return self._start_retry_single_revision(
                unique_ids, user, mode=MODE_STUDY_PLAN
            )
        # cross-paper 路径: paperCode 在跨卷场景语义不适用, 静默忽略 + warn log.
        if paper_code is not None:
            logger.warning(
                "study_plan.cross_paper.paper_code_ignored",
                extra={"paper_code": paper_code, "user_id": user.id},
            )
        return self._start_retry_cross_paper(
            questions, user, mode=MODE_STUDY_PLAN_CROSS_PAPER
        )
