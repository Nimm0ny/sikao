...
- **REQ-5.5** THE SYSTEM SHALL 提供两套字体栈：**UI 默认 = 自托管 `DM Sans` + `Inter` + 系统中文回退**（`"DM Sans", "Inter", "PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", -apple-system, BlinkMacSystemFont, "Segoe UI", system-ui, sans-serif`），代码/快捷键等宽栈用自托管 `JetBrains Mono` + 系统 mono fallback（`"JetBrains Mono", ui-monospace, "SFMono-Regular", Menlo, Consolas, monospace`）；运行时**禁止**引用 `fonts.googleapis.com` / `fonts.gstatic.com` 等外部字体源；`<Numeric>` 组件契约（REQ-8.5）必须使用 `font-variant-numeric: tabular-nums` 保证数字不抖动。
...
| Q3 | REQ-5.5 | 自托管 `DM Sans + Inter + JetBrains Mono` + 系统中文回退 | 运行时禁止 Google Fonts 外链；数字展示 `<Numeric>` 用 `tabular-nums`。 |
...
| R2-Q5 | C.4.2 | `--max-w-workspace = none`（2026-05-27 `SIK-128` Route A supersede） | `workspace` 默认不再承担 1440 cap；1920 主战场按 `viewport - rail-w` 吃满剩余画布。若某 surface 需要窄列，必须显式走 `reading` / `form` / `prose`。 |
