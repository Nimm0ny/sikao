...
**冻结约束**：桌面 sidebar 自 `SIK-121 W5` 起冻结；后续页面不得再改 Rail / nav 结构，只允许补 `Me` 内容。
...
└─ <BottomTabBar h=mobile-bottom-nav-h padding-bottom=safe-bottom>
    └─ 4 个 tab：首页 / 练习 / 复盘 / 笔记
        玻璃拟态 + 自动降级（R1/Q4 决策）；我的入口独立于 navItems
```
...
- [Frontend-IA-V2.md](../../Frontend-IA-V2.md) — IA 历史决策档案（旧 5-tab 信息架构来源，非当前 router authority）
