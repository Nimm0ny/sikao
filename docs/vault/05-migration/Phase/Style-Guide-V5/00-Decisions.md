...
> **来源**：2026-05-23 user 在 `.tmp_review/v5-open-questions.html` / `.tmp_review/v5-open-questions-r2.html` 决策板拍板；**2026-05-24 V5-M0.5 big-bang rebuild 决策**追加更新（影响 R1/Q6）。
> **Last Updated**: 2026-05-24

---

## 0. V5-M0.5 Big-Bang Rebuild（2026-05-24 新增决策）

> **决策来源**：lhr 在 2026-05-24 主动指示 + 拍板（"旧前端不需要修了"+"现在一并清理"）。

### 决策内容

放弃 V4 → V5 整页 surface 切换路径，改为 big-bang 重建：
...
- **R1/Q6（双轨期 2 周）整段 ARCHIVED**
- **REQ-12 整段 ARCHIVED**（详见 `requirements.md` 顶部 UPDATE）
- **design.md §C.6 V4→V5 mapping ARCHIVED**
- **tasks.md Phase 6（21.x）整段 ARCHIVED**
- V5 主线收敛：原 13 milestone 砍到 8 个（详见 `11-Implementation-Plan.md` §V5-M0.5）

### 不变量

INV-1..7 全部不变；R1/Q1..Q5 + R2/Q1..Q6 全部不变；35 组件契约 / 三层 token / 6 桌面页骨架 / 5 类卡片状态机不变。big-bang 改的是"实现路径"，不改"目标 V5 规范"。

### 替代规则

V5 spec 三件套与 Phase 文档落地后，业务 Phase（Home / Practice / Notes / Review / Profile / Marketing）直接消费 V5 规范从零实现。各业务 Phase 前端任务（如 SIK-29 M11/M12 / SIK-26-28 / SIK-66-70 / SIK-53-57）在 V5 框架下重新承接。

详见 [11-Implementation-Plan.md](./11-Implementation-Plan.md) §V5-M0.5 章节。
...
| **R1/Q3** | REQ-5.5 | 自托管 `DM Sans + Inter + JetBrains Mono` + 系统中文回退 | UI 默认：`"DM Sans", "Inter", "PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", -apple-system, BlinkMacSystemFont, "Segoe UI", system-ui, sans-serif`；代码/快捷键等宽：`"JetBrains Mono", ui-monospace, "SFMono-Regular", Menlo, Consolas, monospace`；运行时禁止 Google Fonts 外链；`<Numeric>` 必带 `font-variant-numeric: tabular-nums` |
...
| **R1/Q6** | REQ-12.2 | ~~V4→V5 迁移过渡期 = 2 周（2026-05-23 → 2026-06-06）~~ **ARCHIVED 2026-05-24（V5-M0.5 big-bang）** | ~~双轨期内 V4 token 标 `@deprecated 失效 2026-06-06`；过渡期满 Verifier 跑残留扫描，0 引用才删除~~。big-bang 后无 V4 alias 需要 sunset；V4 token 名在 V5 SSOT 全部消失。 |
...
| **R2/Q5** | C.4.2 / C.4.3 | `--max-w-workspace = none`（2026-05-27 `SIK-128` Route A supersede） | `workspace` 默认不再承担 1440 cap；1920 主战场按 `ws = viewport - rail-w` 吃满剩余画布。若某 surface 需要窄列，必须显式走 `reading` / `form` / `prose`；附加 Rail 折叠规则详见 §C.4.3 |
