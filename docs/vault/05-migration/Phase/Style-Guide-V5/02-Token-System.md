...
## 7. V4 → V5 Token Mapping（双轨期完整去向）

requirements.md REQ-1.6 强制要求**所有 V4 token 在 V5 必须有明确去向**：keep / rename / split / deprecate，不允许"未提及就消失"。下表是完整 mapping。

### 7.1 颜色 / 表面 / 文字（rename 主导）
...
### 7.6 Deprecate 时间线（R1/Q6 决策）

所有 V4 token 在 V5 落地时**同步**标 `@deprecated 2026-05-23 — 失效 2026-06-06`（2 周过渡期）；过渡期满 Verifier 跑 `lint-v4-token-residual.mjs`，0 引用后从 SSOT 删除。详见 [10-Migration.md](./10-Migration.md)。
