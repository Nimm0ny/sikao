...
### 2.5a 字体族（2026-05-28 DM Sans route）

| Token | 值 |
|---|---|
| `--font-family-ui` | `"DM Sans", "Inter", "PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", -apple-system, BlinkMacSystemFont, "Segoe UI", system-ui, sans-serif` |
| `--font-family-ui-secondary` | `"Inter", "DM Sans", "PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", -apple-system, BlinkMacSystemFont, "Segoe UI", system-ui, sans-serif` |
| `--font-family-mono` | `"JetBrains Mono", ui-monospace, "SFMono-Regular", Menlo, Consolas, monospace` |

规则：

- `DM Sans / Inter / JetBrains Mono` 通过 design-system 自托管
- 运行时禁止 Google Fonts 外链
- app 代码不得直写 `font-family`
...
| `--bp-2xl` | 1536 | 1920 主战场（workspace 吃满 Rail 余宽） | 完整 240 | 标准 |
| `--bp-3xl` | 1920 | 超大屏 / 4K | 完整 240 + 不再强制大屏限宽 | `workspace` 跟随共享 token 语义 |
...
| Token | 值 | 用途 |
|---|---|---|
| `--max-w-workspace` | **none** | 共享 entry-workspace 默认值；1920+ 不再自动加 1440 cap |
