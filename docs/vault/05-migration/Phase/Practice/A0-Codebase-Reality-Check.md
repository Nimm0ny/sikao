...
Phase-Home 完成后会建立的基础（Tab 2 直接复用，不重复建）：

| 表 / 模块 | Phase-Home 完工后状态 | Tab 2 在此基础上做什么 |
|---|---|---|
...
| `modules/llm/` | Phase-Home WU-B7 完工后含 plan_generator / plan_adjustor / recommender / 配置 / provider | **WU-B22**：在同一模块内追加 question_generator / essay_grader / reference_answer_generator |
| AuditLogV2 / IdempotencyKeyV2 / LlmCallV2 | Phase-Home WU-B1 已建表；`core/audit.py` 已就位 | 直接复用，不再建 |
| APScheduler 框架 | Phase-Home WU-B8 已就位 | **WU-B23**：注册 4 个新 cron 任务 |

⚠️ **强约束**：Tab 2 启动的硬前置 = Phase-Home WU-B1 + WU-B7 + WU-B9 完工。否则 Tab 2 的 schema 扩展、LLM 复用、API client types 重生成都做不了。
...
├── llm/                      ← Phase-Home WU-B7 完工后状态：含 V2 plan/adjust/recommend
...
modules/llm/
  application/
    plan_generator.py             ← Phase-Home 已建
    plan_adjustor.py              ← Phase-Home 已建
    recommender.py                ← Phase-Home 已建
    recommender_policy.py         ← Phase-Home 已建
    question_generator.py         ← Tab 2 新增（WU-B22.1）
    essay_grader.py               ← Tab 2 新增（WU-B22.2）
    reference_answer_generator.py ← Tab 2 新增（WU-B22.3）
    parsers/
      plan_output_parser.py       ← Phase-Home 已建
      adjustment_parser.py        ← Phase-Home 已建
      recommendation_parser.py    ← Phase-Home 已建
      question_parser.py          ← Tab 2 新增
      grading_parser.py           ← Tab 2 新增
      reference_parser.py         ← Tab 2 新增
    prompts/
      plan_generate.py            ← Phase-Home 已建
      plan_regenerate_range.py    ← Phase-Home 已建
      plan_adjust.py              ← Phase-Home 已建
      recommend_today.py          ← Phase-Home 已建
      _shared.py                  ← Phase-Home 已建
...
| README §6 (本 Phase) | "Tab 2 WU-B22 依赖 Phase-Home WU-B7" | 保留 | 强约束：必须 Phase-Home 完工才能启动 Tab 2 |
