...
- **S3 修订（模考 DRAFT 例外）**：`exam_mode=true` 的 DRAFT 不走 2h 兜底；24h 无活动才转 ABANDONED（reason=`no_activity_mock_draft_24h`）。理由：模考用户可能上午建模考、下班后开始；2h 清理会毁掉整套配置（题目快照 + time_limit + delayed_review）。Stage 3 cleanup SQL 必须含 `exam_mode = false` 排除条件，新增 Stage 3.1 处理 24h 阈值的模考 DRAFT。
- v1 范围内 DRAFT 仅服务于模考（CLP-4）；上述 question_enter / commit_answer 隐式转换是早期残留，留作 v2 扩展。
...
async def cleanup_stale_sessions():
    ...
    # ⚠️ S3 修订：模考 DRAFT 例外（exam_mode=true）不走 2h 兜底
    # 模考用户可能上午建模考、下班后开始；2h 强制清理会毁掉整套配置（题目快照 + time_limit + delayed_review）
    # 模考 DRAFT 治理：仅 24h+ 无任何活动转 ABANDONED（与 IN_PROGRESS 同档），或用户主动 discard
    draft_to_abandoned = await db.execute(
        select(PracticeSessionV2)
        .where(PracticeSessionV2.status == SessionStatus.DRAFT)
        .where(PracticeSessionV2.exam_mode == False)        # 模考 DRAFT 排除
    ...
    # Stage 3.1: 模考 DRAFT 24h 无活动 → ABANDONED（与 IN_PROGRESS 同档）
    mock_draft_to_abandoned = await db.execute(
        select(PracticeSessionV2)
        .where(PracticeSessionV2.status == SessionStatus.DRAFT)
        .where(PracticeSessionV2.exam_mode == True)
        .where(PracticeSessionV2.created_at < now - timedelta(hours=24))
        .limit(500)
    ).scalars().all()
    for s in mock_draft_to_abandoned:
        await transition_in_session(
            s, to=SessionStatus.ABANDONED,
            trigger='no_activity_mock_draft_24h', actor='cron',
            extra={'abandoned_reason': 'no_activity_mock_draft_24h'},
        )
