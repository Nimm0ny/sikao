...
| **R-5** | 毕业语义 | 连续答对 N=2 次自动 graduated + 用户手动 mark_resolved 出口 | SM-2/FSRS 成本高；连续答对是最简硬指标 |
...
| **SRS-7** | 费曼复述加成 | 用户填写 recall_text 后 next_interval *= 2（metadata_json.used_recall=true） | 主动回忆加速巩固 |

### SRS 三档间隔表

```
correct_streak=0  →  next_review_at = now + 1d
correct_streak=1  →  next_review_at = now + 3d
correct_streak=2  →  status = graduated, graduated_at = now
```
...
if correct_streak > 0:
    ...
    correct_streak = 0   # 保持 0 档（next = now + 1d）
```
...
| **AI-Cause-3** | 输出结构 | `{ summary: str(≤200字), dimensions: [{name, severity, suggestion}](≤5项), suggested_actions: [str](≤3项) }` |
...
| **Insights-3** | 错因聚类 | 基于 AiCauseAnalysisV2.result_json.dimensions[].name 标签频次条形图 |
...
| **QHub-2** | ctx 参数 | ?ctx=practice|review|note|favorite|home（决定返回目标 + 重做 source_mode） |
| **QHub-3** | 内容区块 | 题面 + 选项 + 我的全部答题历史 + 正确答案 + 解析 + AI 错因卡 + 题级笔记区 + 关联笔记提示 |
| **QHub-4** | 操作区 | 去重做 / 收藏 / 持久标记 / 加入复盘 / 加入计划 / 已掌握 / 归档 / 写笔记 / 分析错因 |
...
- 数据洞察 `/review/insights`（3 张图）
- 已掌握 `/review/graduated` + 归档区 `/review/archived`
- 题目中枢页 `/q/:id`（跨 tab 共用）
- 错题重做 `/q/:id/redo`（脱壳，复用 PracticeSessionV2）
- AI 错因分析（单题按需 + 多题聚合）
- SRS 排队引擎（简化版 + schema 预留 SM-2）
- 周回顾系统（实时聚合 + cron 预生成 + 一键生成笔记）
- 选项打乱 + 费曼复述输入框
- 跨 tab 联动（写入 / 读取 / 反向流）
...
| 01-Boundary-Rules | PR-R1~R7 逐条引 R/D-R/SRS |
| 02-Data-Model | source_kind 枚举 / SRS 字段 / AiCauseAnalysisV2 结构 |
| 03-Backend-WU | 全部 WU 的决策依据 |
| 04-Frontend-WU | 全部 WU-FR 的决策依据 |
| 05-SRS-Engine | SRS-1~7 |
| 06-AI-Cause-Analysis | AI-Cause-1~10 |
| 07-Smart-Review-Aggregation | D-R4 / D-R5 |
| 08-Question-Hub-Page | QHub-1~7 |
| 09-Cross-Tab-Wiring | Cross-1~8 |
| 10-NonFunctional | D-R12 / D-R13 / AI-Cause-5 |
| 11-Testing | PR-R / SRS invariant |
