...
DeepSeek 官方与阿里百炼平台**都提供 OpenAI 兼容接口**，但 Home Phase **沿用现有 `httpx`-based OpenAI-compatible client**，不引入 `openai` Python SDK。provider 选择在现有 BYOM 抽象之上扩展 system 默认 provider。
...
```
services/api/src/sikao_api/modules/llm/
  __init__.py
  application/
    __init__.py
    usage.py
    ...
    llm/
      __init__.py
      _stub.py                     # 现有 stub，保留
      byom_config.py               # 现有 BYOM 配置，保留并扩展
      conversations.py             # 现有对话历史，保留
      json_parser.py               # 现有 JSON parser，升级
      openai_compatible.py         # 现有 httpx 兼容 client，升级
      pricing.py                   # 现有 token / 成本计算，保留
      provider.py                  # 现有 dataclass / protocol，升级
      ssrf_guard.py                # 现有 SSRF 防护，保留
      usage_estimator.py
      usage_recorder.py
      service.py                   # 新增 facade，外部唯一入口
      plan_generator.py            # 新增 AI 制定 / 局部重生成
      plan_adjustor.py             # 新增 AI 每日调整提案
      recommender.py               # 新增 今日推荐
      recommender_policy.py        # 新增 阈值表（详见 01-Boundary-Rules §2）
      cache.py                     # 新增 进程内 LRU + DB 二级缓存
      cost_tracker.py              # 新增 token 计数 + 成本聚合
      sanitizer.py                 # 新增 用户输入清洗（Infra-PII）
      quotas.py                    # 新增 配额策略
      parsers/
      ...
      prompts/                     # 详见 06-LLM-Prompts.md
        _shared.py                 # 现有，扩展
        study_plan.py              # legacy，保留到 B5.1a 再删
        essay_grading.py           # 现有，不动
        qa.py                      # 现有，不动
        plan_generate.py           # 新增
...
API key 用户本机部署时通过 `.env` 注入。代码层不读取 secret 明文，仅通过 `SecretStr.get_secret_value()` 暴露给现有 provider client。
