...
### 2.3 场景示例：用户加练后 AI 如何响应

**场景**：用户今日计划事件"30 题言语"已 done，正确率 50%。用户主动又做了 30 题（unlinked session，非计划事件触发）。

**系统行为**：

1. **实绩层**：60 题全部计入进度（因 P2），无论是否绑定计划事件
2. **日历视图**：计划事件块显示"✓ done"；额外 30 题作为"实绩块"显示在同一时段（因 P4），视觉区分
3. **进度计算**：今日言语正确率 = 60 题聚合结果（不只看计划内 30 题）
4. **AI 推荐触发**：session.submit 后实时调 recommender（因 B8.4）

**AI 推荐决策树**（recommender 输入信号 → 输出）：

```
输入信号：
├─ 计划层：今日言语事件已 done（30 题）
├─ 实绩层：今日已答 60 题言语，正确率 50%（差目标 25 个百分点）
├─ 实时状态：连续答题时长约 90 分钟，最近 10 题错 6 题（错误率上升）
...
**核心原则**：
- 正确率 < 60% 时 → **优先推复盘**（继续堆量只会强化错误模式）
- 错误率呈上升趋势 → **不再推新题**（疲劳 + 无效重复）
- 推荐永远显示原因（因 Plan-5-Rule 第 3 条）
- 推荐不直接改计划（因 P6）；用户接受后选"进 session"或"加入明天计划"

### 2.4 "仍在学习计划内"的精确语义

用户加练（unlinked session）**不会**被排除在学习计划体系之外。具体含义：

| 维度 | 计划内 session（linked） | 加练 session（unlinked） |
|---|---|---|
| 贡献进度 | ✅ | ✅（因 P2） |
| 影响 AI 推荐 | ✅ | ✅（因 P5，推荐基于全部实绩） |
| 触发 AI 调整提案 | ✅ | ✅（因 ADJ-1，调整基于实绩而非计划事件 status） |
| 显示在日历 | 作为计划事件块 | 作为实绩块（因 P4） |
| 改变计划事件 status | 是（planned → done） | 否（不创建/不改 PlanEventV2） |
| 进入学情分析 | ✅ | ✅ |
