...
| 冲突确认层 | §4 peek-scrim + shadow-l4 | 定义已落（spec design §W3 Conflict Check + Decisions 1 二次确认）：detect 非空 conflicts → 弹层列冲突 → 确认走 commitReschedule / 取消无副作用；detect 请求失败 → fail-fast。实现待 Wave 3 | 定义就绪，待 Wave 3 实现 |
