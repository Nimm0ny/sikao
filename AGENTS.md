...
4. **>100 行改动必 subagent review；>400 行改动再加 master review**：任何新增或修改累计 >100 行的任务，提交前必须经过至少 1 个独立 subagent review；单 subagent 净增 >400 行（含新文件 + 修改）时，提交前 master 还必须亲自读 diff 决定是否接受 / 让另起 review subagent。
