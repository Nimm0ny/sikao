...
window.Buttons = function Buttons() {
  const Btn = ({ variant = 'primary', size = 'md', state = 'default', children, icon, iconRight }) => {
    ...
    const variants = {
      primary: {
        ...
        active: { bg: '#1e40af', fg: '#fff', border: 'transparent' },
        disabled: { bg: '#cbd5e1', fg: '#fff', border: 'transparent' },
      },
      secondary: {
        default: { bg: 'var(--paper)', fg: 'var(--ink)', border: 'var(--line-strong)' },
    ...
    return (
      <button style={{
        ...
        outline: state === 'hover' && variant === 'primary' ? '4px solid rgba(37,99,235,.2)' : 'none',
        fontFamily: 'inherit',
      }}>
  ...
  return (
    <div style={{ width: 1280, height: 720, background: 'var(--paper)', padding: 56, fontFamily: 'var(--sans)', color: 'var(--ink)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
        ...
        <button style={{ width: 40, height: 40, borderRadius: 10, border: '1px solid var(--line-strong)', background: 'var(--paper)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
...
window.Chips = function Chips() {
  const Chip = ({ children, color = 'gray', solid = false, dot = false, dismiss = false }) => {
    ...
    const s = solid ? { bg: { gray: '#0b1120', brand: '#2563eb', success: '#16a34a', warn: '#d97706', danger: '#dc2626' }[color], fg: '#fff', border: 'transparent' } : palette;
    return (
...
window.Inputs = function Inputs() {
  const Field = ({ label, state = 'default', value = '', placeholder = '', help, icon }) => {
    const styles = {
      default: { border: 'var(--line-strong)', bg: 'var(--paper)', help: 'var(--muted)' },
      focus: { border: 'var(--brand)', bg: 'var(--paper)', help: 'var(--brand)', shadow: '0 0 0 4px rgba(37,99,235,.12)' },
      filled: { border: 'var(--line-strong)', bg: 'var(--paper)', help: 'var(--muted)' },
      error: { border: 'var(--danger)', bg: 'var(--paper)', help: 'var(--danger)' },
  ...
  return (
    <div style={{ width: 1280, height: 720, background: 'var(--paper)', padding: 56, fontFamily: 'var(--sans)', color: 'var(--ink)' }}>
      ...
      <div style={{ padding: 14, border: '1px solid var(--brand)', boxShadow: '0 0 0 4px rgba(37,99,235,.12)', borderRadius: 10, minHeight: 100, fontSize: 14, color: 'var(--ink)', background: 'var(--paper)' }}>
      ...
      <div style={{ display: 'flex', gap: 24, alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ width: 18, height: 18, borderRadius: 5, border: '1.5px solid var(--line-strong)', background: 'var(--paper)' }}/>
    ...
    <div style={{ width: 1280, height: 680, background: 'var(--paper)', padding: 56, fontFamily: 'var(--sans)', color: 'var(--ink)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 18, marginTop: 32 }}>
        ...
        <div>
          ...
          <div style={{ background: 'var(--paper)', borderRadius: 16, padding: 24, boxShadow: '0 10px 30px -10px rgb(15 23 42 / .15)' }}>
    ...
    <div style={{ width: 1280, height: 760, background: 'var(--paper)', padding: 56, fontFamily: 'var(--sans)', color: 'var(--ink)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '240px 1fr', gap: 18, marginTop: 28, height: 580 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {/* topbar */}
          <div style={{ height: 60, background: 'var(--paper)', border: '1px solid var(--line)', borderRadius: 12, padding: '0 18px', display: 'flex', alignItems: 'center', gap: 14 }}>
            <div style={{ flex: 1, height: 36, background: 'var(--bg-alt)', borderRadius: 10, padding: '0 14px', display: 'flex', alignItems: 'center', gap: 10, fontSize: 13, color: 'var(--placeholder)' }}>
              ...
              搜索题目、考点...
              <span className="mono" style={{ marginLeft: 'auto', fontSize: 10, color: 'var(--muted)', padding: '2px 6px', background: 'var(--paper)', borderRadius: 4, border: '1px solid var(--line)' }}>⌘ K</span>
            </div>
            <button style={{ width: 36, height: 36, borderRadius: 10, border: '1px solid var(--line)', background: 'var(--paper)', position: 'relative', cursor: 'pointer' }}>
          ...
          <div className="uk-card" style={{ padding: 0 }}>
            ...
            <div style={{ padding: 18, display: 'flex', gap: 14, alignItems: 'center' }}>
              ...
              <div style={{ display: 'inline-flex', padding: 3, background: 'var(--bg-alt)', borderRadius: 10, border: '1px solid var(--line)' }}>
                {['卡片', '列表', '图谱'].map((v, i) => (
                  <span key={v} style={{ padding: '6px 14px', borderRadius: 8, fontSize: 12, fontWeight: 600, color: i === 0 ? 'var(--ink)' : 'var(--muted)', background: i === 0 ? 'var(--paper)' : 'transparent', boxShadow: i === 0 ? '0 1px 2px rgba(0,0,0,.06)' : 'none', cursor: 'pointer' }}>{v}</span>
                ))}
              </div>

              <span style={{ marginLeft: 24, fontSize: 12, fontWeight: 600, color: 'var(--ink-muted)' }}>排序</span>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 12px', background: 'var(--paper)', border: '1px solid var(--line-strong)', borderRadius: 10, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
