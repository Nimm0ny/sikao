...
const Topbar = ({ user = '李思源' }) => (
  <div style={{
    ...
    <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg,#0b1120,#1e293b)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 13 }}>{user[0]}</div>
  </div>
);
          ...
          <div key={i} style={{ position: 'relative', display: 'flex', gap: 28, paddingBottom: 18 }}>
            ...
            <div style={{ position: 'relative', width: 16 }}>
              <div style={{
                ...
                background: s.done ? 'var(--success)' : (s.main ? 'var(--brand)' : 'var(--bg)'),
            ...
            <div className="card" style={{
              ...
              background: s.main ? 'var(--brand)' : (s.done ? 'var(--bg-alt)' : 'var(--bg)'),
