...
window.OptionStates = function OptionStates() {
  const Opt = ({ letter = 'A', text, state }) => {
    const styles = {
      default: { border: 'var(--line-strong)', bg: 'var(--bg)', letterBg: 'var(--bg-alt)', letterFg: 'var(--ink-muted)', fg: 'var(--ink)', icon: null },
      hover: { border: 'var(--brand)', bg: 'var(--bg)', letterBg: 'var(--brand-50)', letterFg: 'var(--brand)', fg: 'var(--ink)', icon: null, shadow: '0 4px 16px -6px rgba(37,99,235,.25)' },
  ...
  return (
    <div style={{ width: 1280, height: 680, background: 'var(--bg)', padding: 56, fontFamily: 'var(--sans)', color: 'var(--ink)', overflow: 'auto' }}>
...
window.AnswerCard = function AnswerCard() {
  ...
  const cell = (s) => {
    const styles = {
      ...
      current: { bg: 'var(--bg)', fg: 'var(--brand)', border: 'var(--brand)', ring: true },
      flagged: { bg: 'var(--warn-bg)', fg: '#a16207', border: '#fde68a' },
      pending: { bg: 'var(--bg)', fg: 'var(--ink-muted)', border: 'var(--line)' },
    };
    return styles[s];
  };
  return (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginTop: 28 }}>
        ...
        <Wrap title="提交失败" k="error">
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
            ...
            <button style={{ padding: '6px 14px', background: 'var(--bg)', color: 'var(--ink-muted)', border: '1px solid var(--line)', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>稍后</button>
          </div>
        </Wrap>
